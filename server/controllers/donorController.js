...
/* 🏕️ Register Donor for Blood Camp */
export const registerForCamp = async (req, res) => {
  try {
    const { campId } = req.params;
    const donorId = req.donor.id;
    const { preferredTime, specialRequests, emergencyContact, emergencyPhone } = req.body;

    // Check if camp exists and is available for registration
    ...
    // Check if camp is upcoming or ongoing
    if (!["Upcoming", "Ongoing"].includes(camp.status)) {
      return res.status(400).json({
        success: false,
        message: "Cannot register for this camp. Camp is either completed or cancelled."
      });
    }
    ...
    if (!donor.isEligible) {
      return res.status(400).json({
        success: false,
        message: "You are not eligible to donate blood. Please wait 90 days from your last donation."
      });
    }

    // Check if donor is already registered for this camp
    // For now, we'll just increment the actualDonors count
    // In production, you might want to create a separate CampRegistration model
    ...
    // Add camp registration to donor's history (optional)
    ...
    // You could add this to a registrations array in donor schema if needed
    // await Donor.findByIdAndUpdate(donorId, {
    //   $push: { campRegistrations: registrationRecord }
    // });
