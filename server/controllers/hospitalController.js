...
/**
 * @desc Create external blood bank request
 * @route POST /api/hospital/external-blood-request
 * @access Private (Hospital)
 */
export const createExternalBloodRequest = async (req, res) => {
  try {
    const hospitalId = req.user._id;
    const { bloodType, units, pincode, externalBanks } = req.body;

    // Validation
    if (!bloodType || !units || !pincode) {
      return res.status(400).json({
        success: false,
        message: "Please provide bloodType, units, and pincode"
      });
    }
    ...
    const externalRequest = {
      ...
      createdAt: new Date(),
      urgency: units >= 5 ? "high" : "medium"
    };

    // Add to hospital's history
    hospital.history = hospital.history || [];
    hospital.history.push({
      eventType: "External Blood Request",
      description: `Created external request for ${units} units of ${bloodType} blood in pincode ${pincode}`,
      date: new Date(),
      referenceId: `EXT-${Date.now()}`
    });

    await hospital.save();

    // Here you could also save to a dedicated ExternalRequest model if needed
    // For now, we're just logging it in hospital history

    res.status(201).json({
      success: true,
      message: "External blood request created successfully",
      data: {
        requestId: `EXT-${Date.now()}`,
        externalRequest,
        contactInfo: externalBanks?.slice(0, 3).map(bank => ({
          name: bank.name,
          phone: bank.phone,
    ...
    console.error("Create external request error:", err);
    res.status(500).json({ 
      success: false, 
      message: "Failed to create external request",
      error: err.message 
    });
  }
};