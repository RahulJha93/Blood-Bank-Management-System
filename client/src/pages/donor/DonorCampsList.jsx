...
import DonationRegistrationModal from "../../components/DonationRegistrationModal";
import {
  MapPin,
  Calendar,
  Clock,
...
const CampCard = ({ camp, onRegister }) => {
  ...
  const canRegister = (isUpcoming || camp.status === 'Ongoing') && !isFull;
      ...
      <div className="pt-4 border-t border-gray-100 flex flex-col justify-between items-start gap-3">
        ...
        <div className="pt-4 border-t border-gray-100 w-full mt-3">
          ...
          {canRegister && (
            <div className="mt-4 pt-3 border-t border-gray-100">
              <button
                ...
                className="w-full bg-red-600 hover:bg-red-700 text-white font-medium py-2.5 px-4 rounded-xl transition-colors flex items-center justify-center gap-2"
              >
          ...
          {isFull && !isCompleted && !isCancelled && (
            <div className="mt-4 pt-3 border-t border-gray-100">
              <div className="w-full bg-gray-100 text-gray-500 font-medium py-2.5 px-4 rounded-xl text-center">
                Camp Full - No Slots Available
              </div>
            </div>
          )}
        </div>
...
export const DonorCampsList = () => {
  ...
  const handleCampRegistration = (camp) => {
    setSelectedCamp(camp);
    setShowRegistrationModal(true);
  };

  // Confirm registration with form data
  const confirmRegistration = async (campId, formData) => {
    const token = localStorage.getItem("token");
    if (!token) {
      toast.error("Authentication required to register for camps.");
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        toast.success(`Successfully registered for ${selectedCamp.title}!`);
        setShowRegistrationModal(false);
        setSelectedCamp(null);
        // Refresh camps to get updated data
        fetchCamps();
      } else {
      ...
      const message = err.response?.data?.message || err.message || "Failed to register for camp.";
      toast.error(message);
    } finally {
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
              {displayedCamps.map((camp) => (
                <CampCard key={camp._id} camp={camp} onRegister={handleCampRegistration} />
              ))}
            </div>
